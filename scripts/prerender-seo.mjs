...
import { CURIOSITIES, CURIOSITY_DATE, CURIOSITY_DESCRIPTION, CURIOSITY_EDITORIAL, curiosityPath, curiositySchema } from '../src/data/curiosities.js'
...
function render({ path, title, description, content, schemas, noindex = false }) {
  ...
  return base
    ...
    .replace('<div id="root"></div>', `<div id="root"><nav aria-label="Principal"><a href="/">Ouvir rádios</a> <a href="/novidades">Novidades</a> <a href="/curiosidades">Curiosidades</a> <a href="/guia/como-ouvir-radio-online">Guia</a> <a href="/relaxar">Relaxar</a> <a href="/sobre.html">Sobre</a></nav>${content}</div>`)
}

function guideLibrary(currentPath) {
  return `<nav class="guide-library" aria-label="Guias para aproveitar a rádio online"><h2>Guias para aproveitar a rádio online</h2><div class="guide-library__grid">${Object.entries(GUIDE_ARTICLES).filter(([path]) => path !== currentPath).map(([path, article]) => `<a href="${path}"><strong>${escape(article.title)}</strong><span>${escape(article.description)}</span></a>`).join('')}</div></nav>`
}

const rootContent = `<main><h1>Rádio FM Online</h1><section><h2>Ouça rádios FM ao vivo</h2><p>Descubra estações de rádio brasileiras e internacionais em um player simples, rápido e com busca por nome, frequência ou cidade. Aproveite uma experiência prática para ouvir rádios FM ao vivo, salvar favoritas e alternar entre gêneros como pop, rock, sertanejo, notícias e internacional.</p><h2>Rádios em destaque</h2><p>Uma rádio por estado brasileiro, em ordem alfabética, para representar a cobertura nacional do catálogo. O restante das ${radios.length} estações está organizado por estado, cidade e gênero logo abaixo.</p>${radioList(getFeaturedRadios())}</section>${exploreCatalogNav()}</main><footer><strong>Rádio FM Online</strong><p>Catálogo independente de estações ao vivo.</p><nav><a href="/guia/como-ouvir-radio-online">Guia</a> <a href="/relaxar">Relaxar</a> <a href="/sobre.html">Sobre</a> <a href="/privacy-policy.html">Privacidade</a> <a href="/terms.html">Termos</a> <a href="/direitos-autorais.html">Direitos autorais</a> <a href="https://wa.me/5511974004755">Fale conosco</a></nav></footer>`
...
const directRoutes = radios.map((radio) => {
  ...
  const listening = LISTENING_DETAILS[radio.id]
  const listeningContent = listening ? `<section>${listening.sections.map((section) => `<h2>${escape(section.title)}</h2><p>${escape(section.text)}</p>`).join('')}<h3>Fontes destas orientações de escuta</h3><p>Consulta das fontes em <time datetime="${listening.reviewedOn}">${listening.reviewedOn.split('-').reverse().join('/')}</time>. As orientações não representam monitoramento ao vivo da programação.</p><ul>${listening.sources.map((source) => `<li><a href="${escape(source.url)}">${escape(source.label)}</a></li>`).join('')}</ul></section>` : ''
  ...
  const editorialProfile = getEditorialProfile(radio.id)
  const editorialSources = editorialProfile
    ? `<h2>Pesquisa e fontes</h2><p>Perfil revisado em ${escape(EDITORIAL_REVIEWED_AT)}, a partir de fontes oficiais da emissora ou organização.</p><ul>${editorialProfile.sources.map((source) => `<li><a href="${escape(source.url)}">${escape(source.label)}</a></li>`).join('')}</ul>`
    : ''
  const content = `<main>${breadcrumbNav(breadcrumbItems)}<article><p>Ouça ao vivo</p><h1>${escape(radio.name)} ao vivo</h1><p>${escape([radio.frequency, radio.city, radio.state, radio.country].filter(Boolean).join(' · '))}</p><h2>Informações da estação</h2><p>${escape(description)}</p><dl>${radio.frequency ? `<dt>Frequência</dt><dd>${escape(radio.frequency)}</dd>` : ''}${radio.band ? `<dt>Banda</dt><dd>${escape(radio.band)}</dd>` : ''}${radio.city ? `<dt>Localidade</dt><dd>${escape([radio.city, radio.state, radio.country].filter(Boolean).join(', '))}</dd>` : ''}${radio.genreLabels.length ? `<dt>Categoria</dt><dd>${escape(radio.genreLabels.join(', '))}</dd>` : ''}</dl>${radio.websiteUrl ? `<a href="${escape(radio.websiteUrl)}">Site oficial</a>` : ''}${hasCityPage && stateSlug ? ` <a href="/${stateSlug}/${citySlug}">${escape(cityLinkLabel)}</a>` : ''}${hasStatePage ? ` <a href="/${stateSlug}">${escape(stateLinkLabel)}</a>` : ''}${profileText ? `<h2>Perfil editorial</h2><p>${escape(profileText)}</p>${editorialProfile?.listeningNote ? `<p>${escape(editorialProfile.listeningNote)}</p>` : ''}${editorialSources}` : ''}${listeningContent}<h2>Frequência da ${escape(radio.name)}</h2><p>${escape(frequencyText || 'Frequência não informada no momento.')}</p><h2>Onde fica a ${escape(radio.name)}?</h2><p>${escape(locationText || 'Localização não informada no momento.')}</p><h2>Qual o estilo da ${escape(radio.name)}?</h2><p>${escape(genreText || 'Estilo não classificado no momento.')}</p><h2>Como ouvir a ${escape(radio.name)} online?</h2><p>${escape(describeHowToListen(radio))} Veja o <a href="/guia/como-ouvir-radio-online">guia completo de como ouvir rádio online</a> para mais detalhes.</p><h2>Rádios relacionadas</h2>${radioList(related)}</article></main>`
  return { path, title: getRadioPageTitle(radio), description, noindex: !editorialProfile, content, schemas: [organization, website, { '@type': 'WebPage', url: `${SITE}${path}`, name: radio.name, isPartOf: { '@id': `${SITE}/#website` } }, breadcrumbSchema(breadcrumbItems), schema] }
})
...
directRoutes.push({ path: '/guia/como-ouvir-radio-online', title: 'Como ouvir rádio online grátis | Rádio FM Online', description: 'Guia completo sobre como ouvir rádio online grátis: pelo celular, computador ou tablet, sem aplicativo, consumo de dados e diferenças em relação ao FM tradicional.', content: guideContent.replace('</main>', `${guideLibrary()}</main>`), schemas: [organization, website, breadcrumbSchema(guideBreadcrumb), guidePageFaqSchema] })

const relaxBreadcrumb = [{ label: 'Início', href: '/' }, { label: 'Relaxar', href: '/relaxar' }]
directRoutes.push({ path: '/relaxar', title: 'Sons para relaxar | Rádio FM Online', description: 'Explore sons de chuva, ondas, ruído grave e tons para meditação gerados no navegador para momentos de relaxamento.', content: `<main>${breadcrumbNav(relaxBreadcrumb)}<article><p>Pausa e concentração</p><h1>Sons para relaxar</h1><p>Escolha sons ambientes sintetizados no navegador para acompanhar momentos de descanso, foco ou meditação. A reprodução começa somente quando você a solicita.</p><h2>Como usar os sons</h2><p>Abra os controles, escolha um ambiente e ajuste o temporizador para encerrar a reprodução automaticamente. Para uma escuta mais confortável, mantenha o volume baixo.</p><p>Os sons são gerados localmente no aparelho; não são gravações baixadas de terceiros.</p></article></main>`, schemas: [organization, website, breadcrumbSchema(relaxBreadcrumb)] })

for (const [path, article] of Object.entries(GUIDE_ARTICLES)) {
  const breadcrumbItems = [{ label: 'Início', href: '/' }, { label: 'Guia', href: '/guia/como-ouvir-radio-online' }, { label: article.title, href: path }]
  const sections = article.sections.map((section, index) => `<section id="passo-${index + 1}"><h2>${escape(section.title)}</h2>${section.paragraphs.map((paragraph) => `<p>${escape(paragraph)}</p>`).join('')}</section>`).join('')
  const contents = `<nav aria-label="Neste guia"><ul>${article.sections.map((section, index) => `<li><a href="#passo-${index + 1}">${escape(section.title)}</a></li>`).join('')}</ul></nav>`
  const sources = article.sources ? `<aside><h2>Referências</h2><ul>${article.sources.map((source) => `<li><a href="${escape(source.url)}">${escape(source.label)}</a></li>`).join('')}</ul></aside>` : ''
  const calculator = article.calculator ? '<section class="data-calculator"><h2>Calcule o consumo de internet</h2><p>A 128 kbps, uma hora equivale a cerca de 57,6 MB; uma hora por dia durante 30 dias equivale a 1,73 GB.</p><p>Fórmula: kbps × horas por dia × dias × 0,45 = MB. A estimativa não inclui imagens, anúncios, protocolos de rede ou outros aplicativos. Ative o JavaScript para alterar os valores na calculadora.</p></section>' : ''
  const content = `<main><div class="direct-article-page">${breadcrumbNav(breadcrumbItems)}<article><p>Guia prático</p><h1>${escape(article.title)}</h1><p>${escape(article.lead)}</p>${contents}${calculator}${sections}${sources}${guideLibrary(path)}<h2>Continue explorando</h2><p>Veja também o <a href="/guia/como-ouvir-radio-online">guia para ouvir rádio online</a>.</p></article></div></main>`
  directRoutes.push({ path, title: `${article.title} | Rádio FM Online`, description: article.description, content, schemas: [organization, website, breadcrumbSchema(breadcrumbItems)] })
}
...
for (const route of taxonomyRoutes) {
  ...
  const locationSection = locationBreakdown.length > 0
    ...
    : ''
  const cityEditorial = isCity && route.state ? getCityEditorial(slugify(route.state), slugify(route.name)) : null
  const citySummary = cityEditorial ? getCityCatalogSummary(route.items) : null
  const cityIsEditoriallyReady = !isCity || (route.state && isCityEditorialReady(slugify(route.state), slugify(route.name)))
  const citySections = cityEditorial?.sections?.map((section) => `<section><h3>${escape(section.title)}</h3>${section.paragraphs.map((paragraph) => `<p>${escape(paragraph)}</p>`).join('')}</section>`).join('') || ''
  const cityEditorialSection = cityEditorial && citySummary
...
const curiosityCards = (items) => items.map((article) => `<article><p>${escape(article.category)}</p><h3><a href="${curiosityPath(article)}">${escape(article.title)}</a></h3><p>${escape(article.description)}</p></article>`).join('')
directRoutes.push({ path: '/curiosidades', title: 'Curiosidades da música e do rádio | Rádio FM Online', description: CURIOSITY_DESCRIPTION, schemas: [organization, website], content: `<main><h1>Curiosidades da música e do rádio</h1><p>${escape(CURIOSITY_DESCRIPTION)}</p>${curiosityCards(CURIOSITIES)}<aside><h2>Como ler esta seleção</h2><p>Cada matéria apresenta uma pergunta, explica os critérios e indica as fontes. Listas históricas mantêm sua data de referência: popularidade, qualidade artística e influência cultural são conceitos diferentes.</p><p>${escape(CURIOSITY_EDITORIAL)}</p><a href="/sobre.html">Conheça o projeto e entre em contato</a></aside></main>` })
for (const article of CURIOSITIES) {
  const path = curiosityPath(article)
  const sections = article.sections.map((section, index) => `<section id="secao-${index + 1}"><h2>${escape(section.title)}</h2>${section.paragraphs.map((paragraph) => `<p>${escape(paragraph)}</p>`).join('')}${section.orderedItems ? `<ol>${section.orderedItems.map((item) => `<li>${escape(item)}</li>`).join('')}</ol>` : ''}</section>`).join('')
  const sources = article.sources.map((source) => `<li><a href="${escape(source.url)}">${escape(source.label)}</a></li>`).join('')
  const contents = `<nav aria-label="Nesta matéria"><ul>${article.sections.map((section, index) => `<li><a href="#secao-${index + 1}">${escape(section.title)}</a></li>`).join('')}</ul></nav>`
