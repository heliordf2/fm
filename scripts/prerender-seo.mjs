...
for (const [path, article] of Object.entries(GUIDE_ARTICLES)) {
  ...
  const sections = article.sections.map((section) => `<section><h2>${escape(section.title)}</h2>${section.paragraphs.map((paragraph) => `<p>${escape(paragraph)}</p>`).join('')}</section>`).join('')
  const content = `<main><div class="direct-article-page">${breadcrumbNav(breadcrumbItems)}<article><p>Guia prático</p><h1>${escape(article.title)}</h1><p>${escape(article.lead)}</p>${sections}<h2>Continue explorando</h2><p>Veja também o <a href="/guia/como-ouvir-radio-online">guia para ouvir rádio online</a>.</p></article></div></main>`
