...
import DataUsageCalculator from '../components/DataUsageCalculator.jsx'
...
import { CATALOG_REVIEWED_AT, EDITORIAL_REVIEWED_AT, GENRE_DESCRIPTIONS, GENRE_LABELS, describeCityInsight, describeFrequency, describeGenre, describeHowToListen, describeLocation, describeStationProfile, getAllRadios, getEditorialProfile, getIndexableCities, getIndexableStates, getLocationBreakdown, getRadioByPath, getRadioMetaDescription, getRadioPageTitle, getRadiosByCity, getRadiosByGenre, getRelatedRadios, getStateArticle, isIndexableListing, slugify } from '../data/radioRepository.js'
...
function RadioPage({ radio, player, favorites, hiddenState }) {
  ...
  usePageSeo({ title: getRadioPageTitle(radio), description, path, noindex: !editorialProfile, schemas })
...
function TaxonomyPage({ config, path, player, favorites, hiddenState }) {
  ...
  const cityEditorial = isCity && config.state ? getCityEditorial(slugify(config.state), slugify(config.name)) : null
  const cityCatalogSummary = cityEditorial ? getCityCatalogSummary(radios) : null
  ...
  const cityIsEditoriallyReady = !isCity || (config.state && isCityEditorialReady(slugify(config.state), slugify(config.name)))
  usePageSeo({ title, description, path, noindex: !cityIsEditoriallyReady || (isState && !isIndexableListing(radios)), schemas })
  ...
  return <main className="direct-main"><Breadcrumb items={breadcrumbItems} /><header className="direct-intro"><p className="direct-kicker">{kicker}</p><h1>{h1}</h1><p>{description} A ordem não representa audiência nem popularidade.</p></header><DirectRadioGrid radios={radios} player={player} favorites={favorites} hiddenState={hiddenState} /><section className="direct-copy"><div><h2>{howToHeading}</h2><p>{howToText}</p>{insightSentence && <p>{insightSentence}</p>}</div></section>{cityEditorial && cityCatalogSummary && <section className="direct-city-editorial"><p className="direct-kicker">Leitura do catálogo</p><h2>{cityEditorial.title}</h2><p>{cityEditorial.introduction}</p><p>Há <strong>{cityCatalogSummary.count} rádios cadastradas</strong> nesta cidade. Os formatos mais presentes são {cityCatalogSummary.topFormats.join(', ')}.</p><p>Os números e formatos desta seção são calculados a partir do catálogo; a programação ao vivo pode mudar e deve ser confirmada na fonte oficial da emissora.</p>{cityEditorial.sections?.map((section) => <section key={section.title} className="direct-city-editorial__section"><h3>{section.title}</h3>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}{cityEditorial.sources?.length > 0 && <aside className="direct-editorial-sources"><h3>Fontes oficiais consultadas</h3><ul>{cityEditorial.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noopener noreferrer">{source.label}</a></li>)}</ul></aside>}</section>}{locationBreakdown.length > 0 && <section className="direct-article"><h2>{locationHeading}</h2>{genreDescription && <p>{genreDescription}</p>}<ul>{locationBreakdown.map((entry) => {
...
function ArticleGuidePage({ path, article }) {
  const breadcrumbItems = [{ label: 'Início', href: '/' }, { label: 'Guia', href: '/guia/como-ouvir-radio-online' }, { label: article.title, href: path }]
  usePageSeo({ title: `${article.title} | Rádio FM Online`, description: article.description, path, schemas: [ORGANIZATION_SCHEMA, WEBSITE_SCHEMA, breadcrumbSchema(breadcrumbItems)] })
  return <main className="direct-main"><div className="direct-article-page"><Breadcrumb items={breadcrumbItems} /><article className="direct-article"><p className="direct-kicker">Guia prático</p><h1>{article.title}</h1><p className="direct-lead">{article.lead}</p><nav aria-label="Neste guia"><ul>{article.sections.map((section, index) => <li key={section.title}><a href={`#passo-${index + 1}`}>{section.title}</a></li>)}</ul></nav>{article.calculator && <DataUsageCalculator />}{article.sections.map((section, index) => <section id={`passo-${index + 1}`} key={section.title}><h2>{section.title}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}{article.sources && <aside><h2>Referências</h2><ul>{article.sources.map((source) => <li key={source.url}><a href={source.url} target="_blank" rel="noopener noreferrer">{source.label}</a></li>)}</ul></aside>}<GuideLibrary currentPath={path} /><h2>Continue explorando</h2><p>Veja também o <a href="/guia/como-ouvir-radio-online">guia para ouvir rádio online</a> e navegue pelas páginas de <a href="/genero/noticias">notícias</a>, <a href="/genero/pop">pop</a> e <a href="/sao-paulo/sao-paulo">rádios de São Paulo</a>.</p></article></div></main>
}
